                            ...
                            <div class="col-sm-10">
                                <select name="category_id"
                                    ...
                                    @foreach (App\Models\Category::all() as $category)
                                        <option value="{{ $category->id }}" {{ (isset($machine->category) && $machine->category->id == $category->id) ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-10">
                                <select name="category_type_id"
                                    ...
                                    @foreach (App\Models\CategoryType::all() as $type)
                                        <option required value="{{ $type->id }}" {{ (isset($machine->type) && $machine->category_type_id == $type->id) ? 'selected' : '' }}>
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-10">
                                <select name="model_id" class="form-control border-0 border-bottom rounded-0 shadow-none">
                                    ...
                                    @foreach (App\Models\MachineModel::all() as $model)
                                        <option required value="{{ $model->id }}" {{ (isset($machine->model) && $machine->model->id == $model->id) ? 'selected' : '' }}>
                                            {{ $model->number }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-10">
                                <select name="fuel_id" class="form-control border-0 border-bottom rounded-0 shadow-none">
                                    ...
                                    @foreach (App\Models\Fuel::all() as $fuel)
                                        <option required value="{{ $fuel->id }}" {{ (isset($machine->$fuel) && $machine->fuel_id == $fuel->id) ? 'selected' : '' }}>
                                            {{ $fuel->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-sm-10">
                                <select name="brand_id" class="form-control border-0 border-bottom rounded-0 shadow-none">
                                    ...
                                    @foreach (App\Models\Brand::all() as $brand)
                                        <option required value="{{ $brand->id }}" {{ (isset($machine->brand) && $machine->brand->id == $brand->id) ? 'selected' : '' }}>
                                            {{ $brand->name }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="tab-content" id="myTabContent">
                        ...
                        <div class="tab-pane fade show active" id="features" role="tabpanel" aria-labelledby="features-tab">
                            <table class="table table-bordered" id="featuresTable">
                                ...
                                <tbody>
                                    @php
                                        $machineFeatures = old('features') ? json_decode(old('features'), true) : ($machine->machineFeatures ?? []);
                                    @endphp
                                    @if(!empty($machineFeatures))
                                        @foreach($machineFeatures as $feature)
                                            <tr>
                                                <td contenteditable="true" class="feature-name">{{ $feature['name'] ?? '' }}</td>
                                                <td contenteditable="true" class="feature-value">{{ $feature['value'] ?? '' }}</td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr>
                        ...
                        <div class="tab-pane fade" id="properties" role="tabpanel" aria-labelledby="properties-tab">
                            <table class="table table-bordered" id="propertiesTable">
                                ...
                                <tbody>
                                    @php
                                        $machineProperties = old('properties') ? json_decode(old('properties'), true) : ($machine->machineProperties ?? []);
                                    @endphp
                                    @if(!empty($machineProperties))
                                        @foreach($machineProperties as $property)
                                            <tr>
                                                <td contenteditable="true" class="property-name">{{ $property['name'] ?? '' }}</td>
                                                <td contenteditable="true" class="property-value">{{ $property['value'] ?? '' }}</td>
                                            </tr>
                                        @endforeach
                                    @else
                                        <tr>
                            ...
                            <div id="photoPreview" class="d-flex flex-wrap gap-2">
                                @if(isset($machine->pictures) && count($machine->pictures))
                                    @foreach($machine->pictures as $photo)
                                        <img src="{{ asset('storage/' . $photo->img_url) }}" class="img-thumbnail" style="width:100px;height:100px;object-fit:cover;" alt="Photo">
                                    @endforeach
                                @endif
                            </div>
                        </div>
